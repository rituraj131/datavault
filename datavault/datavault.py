...
class DataVault(object):
        ...
        try:
            ...
            if command.var_name in self.item_store:
                ...
                if caller != "admin":
                    ...
                    cmd = SetDelegation(command.var_name, "admin", "write", caller)
                    response = self._run_set_delegation("admin", cmd)

                    if response != "SET_DELEGATION":
                        logging.error('Oops, got an invalid response: {}'.format(response))

                    cmd = SetDelegation(command.var_name, "admin", "read", caller)
                    self._run_set_delegation("admin", cmd)

                    if response != "SET_DELEGATION":
                        logging.error('Oops, got an invalid response: {}'.format(response))

                    cmd = SetDelegation(command.var_name, "admin", "append", caller)
                    self._run_set_delegation("admin", cmd)

                    if response != "SET_DELEGATION":
                        logging.error('Oops, got an invalid response: {}'.format(response))

                    cmd = SetDelegation(command.var_name, "admin", "delegation", caller)
                    self._run_set_delegation("admin", cmd)
    ...
    def _run_local(self, caller, command):
        try:
            if command.var_name not in self.item_store:
                ...
                response = self._run_set_delegation("admin", cmd)
                if response.status != "SET_DELEGATION":
                    logging.error('Oops, got an invalid response: {}'.format(response.status))
    ...
    def _run_append_to(self, caller, command):
        ...
        # either write or append permission is required to append
                                      ...
                                      self.user_store[caller]) \
                and not self._has_permission("append", self.item_store[command.var_name],
                                             self.user_store[caller]):
            return Response('DENIED')
        else:
            try:
    ...
    def _run_set_delegation(self, caller, command):
        if command.source_user not in self.user_store or command.target_user not in self.user_store:
            return Response('FAILED')
        elif command.target != 'all' and self.item_store[command.target].scope == "local" or command.target not in self.item_store:
            return Response('FAILED')
            ...
            return Response('DENIED')
                ...
                not self._has_permission('delegate', self.item_store[command.target],
                                        self.user_store[command.source_user]):
            return Response('DENIED')  # Fix "break16"
        else:
    ...
    def _run_delete_delegation(self, caller, command):
        if command.source_user not in self.user_store or command.target_user not in self.user_store:
            ...
            return Response('DENIED')
        elif caller == command.source_user and self._has_permission('delegate', self.item_store[command.target],
                                                                    self.user_store[command.source_user]) is False:
            return Response('DENIED')
        else:
            # Only of these will be set at any given time
            permission = command.permission.read or \
                         command.permission.write or \
                         command.permission.append or \
                         command.permission.delegate
            if command.target == "all":
                for item in self._get_items_with_this_permission(command.permission, self.user_store[command.source_user]):
                    command.target = item
                ...
                for usr in self.user_store:
                    command.target_user = usr
                    self._run_delete_delegation(caller, command)
            elif caller == 'admin' or self._has_permission('delegate', self.item_store[command.target],
                                                           self.user_store[command.source_user]):
                self.permissions.revoke_permission(self.user_store[command.target_user], self.item_store[command.target],
                                                   permission)
